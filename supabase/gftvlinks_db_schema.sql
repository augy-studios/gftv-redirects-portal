...
-- gftvlinks_backup_codes table
create table public.gftvlinks_backup_codes (
  id uuid not null default gen_random_uuid (),
  user_id uuid not null,
  code_hash text not null,
  created_at timestamp with time zone not null default now(),
  constraint gftvlinks_backup_codes_pkey primary key (id),
  constraint gftvlinks_backup_codes_user_id_fkey foreign KEY (user_id) references gftvlinks_users (id) on delete CASCADE
) TABLESPACE pg_default;

create index IF not exists idx_backup_codes_user on public.gftvlinks_backup_codes using btree (user_id) TABLESPACE pg_default;
...
create table public.gftvlinks_links (
  ...
  constraint gftvlinks_links_user_id_fkey foreign KEY (user_id) references gftvlinks_users (id) on delete CASCADE
) TABLESPACE pg_default;
...
create table public.gftvlinks_ownership_requests (
  ...
  constraint gftvlinks_ownership_requests_owner_id_fkey foreign KEY (owner_id) references gftvlinks_users (id) on delete CASCADE,
  constraint gftvlinks_ownership_requests_requester_id_fkey foreign KEY (requester_id) references gftvlinks_users (id) on delete CASCADE
) TABLESPACE pg_default;
...
create table public.gftvlinks_profileviews (
  ...
  constraint gftvlinks_profileviews_viewed_id_fkey foreign KEY (viewed_id) references gftvlinks_users (id) on delete CASCADE,
  constraint gftvlinks_profileviews_viewer_id_fkey foreign KEY (viewer_id) references gftvlinks_users (id) on delete CASCADE
) TABLESPACE pg_default;
...
-- gftvlinks_sessions table
create table public.gftvlinks_sessions (
  id uuid not null default gen_random_uuid (),
  user_id uuid null,
  token text not null,
  expires_at timestamp with time zone not null,
  created_at timestamp with time zone null default now(),
  constraint gftvlinks_sessions_pkey primary key (id),
  constraint gftvlinks_sessions_token_key unique (token),
  constraint gftvlinks_sessions_user_id_fkey foreign KEY (user_id) references gftvlinks_users (id) on delete CASCADE
) TABLESPACE pg_default;

create index IF not exists idx_sessions_token on public.gftvlinks_sessions using btree (token) TABLESPACE pg_default;

create index IF not exists idx_sessions_user_id on public.gftvlinks_sessions using btree (user_id) TABLESPACE pg_default;

-- gftvlinks_totp_challenges table
create table public.gftvlinks_totp_challenges (
  token text not null,
  user_id uuid not null,
  expires_at timestamp with time zone not null default (now() + '00:10:00'::interval),
  constraint gftvlinks_totp_challenges_pkey primary key (token),
  constraint gftvlinks_totp_challenges_user_id_fkey foreign KEY (user_id) references gftvlinks_users (id) on delete CASCADE
) TABLESPACE pg_default;

create index IF not exists idx_totp_challenges_user on public.gftvlinks_totp_challenges using btree (user_id) TABLESPACE pg_default;

-- gftvlinks_trusted_devices table
create table public.gftvlinks_trusted_devices (
  id uuid not null default gen_random_uuid (),
  user_id uuid not null,
  device_token text not null,
  expires_at timestamp with time zone not null default (now() + '30 days'::interval),
  constraint gftvlinks_trusted_devices_pkey primary key (id),
  constraint gftvlinks_trusted_devices_device_token_key unique (device_token),
  constraint gftvlinks_trusted_devices_user_id_fkey foreign KEY (user_id) references gftvlinks_users (id) on delete CASCADE
) TABLESPACE pg_default;

create index IF not exists idx_trusted_devices_token on public.gftvlinks_trusted_devices using btree (device_token) TABLESPACE pg_default;

create index IF not exists idx_trusted_devices_user on public.gftvlinks_trusted_devices using btree (user_id) TABLESPACE pg_default;

create index IF not exists idx_trusted_devices_user_id on public.gftvlinks_trusted_devices using btree (user_id) TABLESPACE pg_default;

create index IF not exists idx_trusted_devices_device_token on public.gftvlinks_trusted_devices using btree (device_token) TABLESPACE pg_default;

-- gftvlinks_users table
create table public.gftvlinks_users (
  id uuid not null default gen_random_uuid (),
  ...
  constraint gftvlinks_users_pkey primary key (id),
  constraint gftvlinks_users_email_key unique (email),
  constraint gftvlinks_users_username_key unique (username)
) TABLESPACE pg_default;
...
create table public.gftvlinks_users_preapproved (
  ...
  constraint gftvlinks_users_preapproved_preapproved_by_fkey foreign KEY (preapproved_by) references gftvlinks_users (id) on delete set null,
  constraint gftvlinks_users_preapproved_user_id_fkey foreign KEY (user_id) references gftvlinks_users (id) on delete set null,
