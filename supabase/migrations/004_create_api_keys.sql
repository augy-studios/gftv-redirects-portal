...
CREATE TABLE IF NOT EXISTS gftvlinks_api_keys (
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_api_keys_user_id ON gftvlinks_api_keys(user_id);

-- Index for efficient lookup by api_key during API requests
CREATE INDEX IF NOT EXISTS idx_api_keys_api_key ON gftvlinks_api_keys(api_key);
