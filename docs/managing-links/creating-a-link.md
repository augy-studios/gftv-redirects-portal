...
{% hint style="warning" %}
Destination URLs cannot point back to `gftv.asia` itself. Self-referential redirects are blocked to prevent loops. The exceptions are `guide.gftv.asia` and `form.gftv.asia`, which are allowed as destinations.
{% endhint %}
